...
# File:       ThreeReasons_SEM_Jan2025.R
# Date:       23-Jul-2024
#
# Purpose:    Run SEM model to test Intergenerational Transmission & Assortative Mating in Dunedin
...
MHdx <- read_csv("C:/Users/rh93/Box/Duke_DPPPlab/Renate2015/2024_ThreeReasons/Dunedin/FamilyHx_13Apr2020.csv") %>%
          select(snum, anyint_psm, anyint_bmum, anyint_bdad, anyint_matgm, anyint_matgf, anyint_patgm, anyint_patgf,
                       anyext_psm, anyext_bmum, anyext_bdad, anyext_matgm, anyext_matgf, anyext_patgm, anyext_patgf,
                       anythd_psm, anythd_bmum, anythd_bdad, anythd_matgm, anythd_matgf, anythd_patgm, anythd_patgf) %>%
          rename(c_ext = anyext_psm,
                 c_int = anyint_psm,
                 c_thd = anythd_psm,
                 m_ext = anyext_bmum,
                 m_int = anyint_bmum,
...
write_csv(as.data.frame(polycors), "C:/Users/rh93/Box/Duke_DPPPlab/Renate2015/2024_ThreeReasons/Dunedin/SEM_polycors_All.csv")
...
fit_thrsh <- sem(model = c(misc, 
                            thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, thrsh_gm_e, thrsh_gm_t, thrsh_gf), 
                  data = MHdx, 
                  missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                  ordered = c("c_ext",  "c_int",  "c_thd",
                              "m_ext",  "m_int",  "m_thd",
...
fit_cm_mf <- sem(model = c(misc, 
                           thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, thrsh_gm_e, thrsh_gm_t, thrsh_gf, com_mf), 
                 data = MHdx, 
                 missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                 ordered = c("c_ext",  "c_int",  "c_thd",
                             "m_ext",  "m_int",  "m_thd",
...
cm_mf_fit <- fitMeasures(fit_cm_mf, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
gp_cm_mf_test <- lavTestLRT(fit_thrsh, fit_cm_mf)


fit_cm_gm <- sem(model = c(misc, 
                           thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, thrsh_gm_e, thrsh_gm_t, thrsh_gf, com_gm), 
                 data = MHdx, 
                 missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                 ordered = c("c_ext",  "c_int",  "c_thd",
                             "m_ext",  "m_int",  "m_thd",
...
cm_gm_fit <- fitMeasures(fit_cm_gm, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
gp_cm_gm_test <- lavTestLRT(fit_thrsh, fit_cm_gm)

fit_cm_gf <- sem(model = c(misc, 
                           thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, thrsh_gm_e, thrsh_gm_t, thrsh_gf, com_gf), 
                 data = MHdx, 
                 missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                 ordered = c("c_ext",  "c_int",  "c_thd",
                             "m_ext",  "m_int",  "m_thd",
...
cm_gf_fit <- fitMeasures(fit_cm_gf, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
gp_cm_gf_test <- lavTestLRT(fit_thrsh, fit_cm_gf)

fit_cm_gp <- sem(model = c(misc, 
                           thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, thrsh_gm_e, thrsh_gm_t, thrsh_gf, com_gp), 
                 data = MHdx, 
                 missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                 ordered = c("c_ext",  "c_int",  "c_thd",
                             "m_ext",  "m_int",  "m_thd",
...
cm_gp_fit <- fitMeasures(fit_cm_gp, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
gp_cm_gp_test <- lavTestLRT(fit_thrsh, fit_cm_gp)
...
cm_mfgp_fit <- fitMeasures(fit_cm_mfgp, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
gp_cm_mfgp_test <- lavTestLRT(fit_thrsh, fit_cm_mfgp)
...
cm_mfgp1_fit <- fitMeasures(fit_cm_mfgp1, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
gp_cm_mfgp1_test <- lavTestLRT(fit_thrsh, fit_cm_mfgp1)

fit_cm_mfgp2 <- sem(model = c(misc, 
                    thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, thrsh_gm_e, thrsh_gm_t, thrsh_gf, com_mf, com_gm, com_gf), 
                    data = MHdx, 
                    missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                    ordered = c("c_ext",  "c_int",  "c_thd",
                                "m_ext",  "m_int",  "m_thd",
...
cm_mfgp2_fit <- fitMeasures(fit_cm_mfgp2, 
                            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
                            output = "vector")
gp_cm_mfgp2_test <- lavTestLRT(fit_thrsh, fit_cm_mfgp2)

tests <- c("CM1: MF", "CM2: GM", "CM3: GF", "CM4: GP", "CM5: MFGP", "CM6: MF + GP", "CM7: MH + GM + GF")

comorbidity_fit <- bind_rows(cm_mf_fit, cm_gm_fit, cm_gf_fit, cm_gp_fit, 
                             cm_mfgp_fit, cm_mfgp1_fit, cm_mfgp2_fit) %>%
                   bind_cols(tests) %>%
                   rename(Test = `...10`)

comorbidity_test <- bind_rows(gp_cm_mf_test, gp_cm_gm_test, gp_cm_gf_test, gp_cm_gp_test,
                              gp_cm_mfgp_test, gp_cm_mfgp1_test, gp_cm_mfgp2_test)
comorbidity_test <- comorbidity_test[c(1,2,4,6,8,10,12,14),]
...
fit_am_mf <- sem(model = c(misc, 
                           thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, 
                           thrsh_gm_e, thrsh_gm_t, thrsh_gf, com_mf, com_gm, com_gf, am_mf), 
                    data = MHdx, 
                    missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                    ordered = c("c_ext",  "c_int",  "c_thd",
                                "m_ext",  "m_int",  "m_thd",
...
am_mf_fit <- fitMeasures(fit_am_mf, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
cm_am_mf_test <- lavTestLRT(fit_cm_mfgp2, fit_am_mf)

fit_am_gp <- sem(model = c(misc, 
                           thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, 
                           thrsh_gm_e, thrsh_gm_t, thrsh_gf, com_mf, com_gm, com_gf, am_gp), 
                 data = MHdx, 
                 missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                 ordered = c("c_ext",  "c_int",  "c_thd",
                             "m_ext",  "m_int",  "m_thd",
...
am_gp_fit <- fitMeasures(fit_am_gp, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
cm_am_gp_test <- lavTestLRT(fit_cm_mfgp2, fit_am_gp)

fit_am_mfgp <- sem(model = c(misc, 
                             thrsh, wi_ind, as_mat, ig12, ig23m, ig23p, thrsh_gm_e, thrsh_gm_t, thrsh_gf, 
                             com_mf, com_gm, com_gf,
                             am_mf, am_gp), 
                 data = MHdx, 
                 missing = "pairwise", estimator = "WLSMV", parameterization = "theta",
                 ordered = c("c_ext",  "c_int",  "c_thd",
...
am_mfgp_fit <- fitMeasures(fit_am_mfgp, 
            c("chisq", "df", "pvalue", "cfi", "tli", "srmr", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper"),
            output = "vector")
cm_am_mfgp_test <- lavTestLRT(fit_cm_mfgp2, fit_am_mfgp)
...
FinalParameters <- parameterEstimates(fit_gen3)

write_csv(all_fit, "C:/Users/rh93/Box/Duke_DPPPlab/Renate2015/2024_ThreeReasons/Dunedin/ModelFitStats_Jan2025_Misc.csv")
write_csv(all_test, "C:/Users/rh93/Box/Duke_DPPPlab/Renate2015/2024_ThreeReasons/Dunedin/ModelComparisons_Jan2025_Misc.csv")
write_csv(FinalParameters, "C:/Users/rh93/Box/Duke_DPPPlab/Renate2015/2024_ThreeReasons/Dunedin/ModelParams_Jan2025_Misc.csv")
