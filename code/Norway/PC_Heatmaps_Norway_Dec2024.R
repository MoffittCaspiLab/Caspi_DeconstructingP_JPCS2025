...
Either <- read_dat(OR_file = "ORs_EitherParent_Dec2024.csv") %>% rename(Value = Estimate)
Mother <- read_dat(OR_file = "ORs_Mother_Dec2024.csv") %>% rename(Value = Estimate)
Father <- read_dat(OR_file = "ORs_Father_Dec2024.csv") %>% rename(Value = Estimate)
Random <- read_dat(OR_file = "ORs_RandomParent_Dec2024.csv") %>% rename(Value = Estimate)

NoPCom <- read_dat(OR_file = "ORs_EitherNoPCom_Dec2024.csv") %>% rename(Value = Estimate) %>% distinct(.keep_all = TRUE)
...
Either_HM <- make_heatmap(Either_flip, low = 0.8)
Mother_HM <- make_heatmap(Mother_flip, low = 0.8)
Father_HM <- make_heatmap(Father_flip, low = 0.8)
Random_HM <- make_heatmap(Random_flip, low = 0.4)
...
readr::write_file(Either_HM, "Either_Dec2024.html")
readr::write_file(Mother_HM, "Mother_Dec2024.html")
readr::write_file(Father_HM, "Father_Dec2024.html")
readr::write_file(Random_HM, "Random_Dec2024.html")

## Function to Create OR Heat map
make_heatmap1 <- function(df, low) {
...
NoPCom_HM <- make_heatmap1(NoPCom_flip, low = 0.8)
NoPCom_HM

readr::write_file(NoPCom_HM, "NoPCom_Dec2024.html")

## Descriptive table
descriptive <- read.csv("ParentChild_Prevalence_Dec2024.csv") %>%
